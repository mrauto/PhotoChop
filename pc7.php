<?php 
/* pc7.php Sep  2010  ver 0.3.7.0  
 * 
 * Sep attempt to introduce variable width for existing frames "fitframe"
 * july30 attempt to suppress out/ , increment page wide by drop down , detect zip extension existence etc...
 ...
 *
 * todo: find pc7.php in FORM and put a self reference instead
 */
 
//ini_set('memory_limit','1024M'); //for php fatal error memory exhausted 192>256 for c24.jpg (squarepic 5000x5000 edith who else)
...
// feeding by request (get or post) will override existing values, of course
...
if (!isset($apar)) {
	...
	$upname = $apar['upname'];  
	$pathout = 'out/'; // $apar['pathout'];  // force out/ useless to parameterize
	$nw = $apar['nw'];  
	$wframe = $apar['wframe'];  
	$ox = $apar['ox'];  
...
persists($filename, 'filename', 'demo/7.jpg'); 
persists($urlname, 'urlname', 'demo/7.jpg'); 
persists($upname, 'upname', 'album/8.jpg'); 
persists($pathout, 'pathout', 'out/');  
persists($w0, 'w0', 3000); 
persists($h0, 'h0', 2000); 
persists($nw,   'nw', 3);
persists($wframe,   'wframe', 11.0);
persists($ox,   'ox', 0.1);
...
// Selection of source file from 3 inputs // now 2 100730
...
// removed cropping 100912 but keep this just in case
$cropchecked = ' ';
$crop = '';
/* checkbox for   crop    //removed cropping 100912
...
// radio button for frame fit    
if (isset($_REQUEST['fit']) && $_REQUEST['fit'] =='frame' ) { // new pic > no crop!  && !$newpic 
...
// if wframe is <> 0 the nw should be calculated from wframe
if ( $framechecked == 'checked' ) {
	$nwstar0 = $wframe/$wp; // theoretical number of prints to cover ex 7in/6in = 1.1 pic wide
	$nw = ceil( $nwstar0 ) ; // ex 7/6 = 1.1 , ceil-> nw = 2 actually needed
	$framefit = true;
} else {
	$framefit = false;
	$nwstar0 = $nw;
}
// scope - nothing to do with pixel size of output print
// w = pixel width of each elemental rectangle to be taken out of the original (input) pic
// equals pixel width of source picture divided by n factor plus a small scaling factor allowing for small overlay
// if framefit=true the scope is virtually increased by the virtual reduction in print width
// $w = ceil( ($w0 / $nw) * (1 + ($ox/$wp) * ($nw-1)/$nw )  ); // overlay x nwstar0
$w = ceil( ($w0 / $nwstar0) * (1 + ($ox/$wp) * ($nw-1)/$nw )  ); // overlay x nwstar0
$h = ceil( $w * $hp / $wp );
...
// 100912 try to simulate framefit
//
$src_image = imagecreatefromjpeg('mini/minipic.jpg');
for ($j=0; $j < $nh; $j++) {
	for ($i=0; $i < $nw; $i++) {
		$x = $i * $w1 / $nwstar0 ; //no overlap management necessary
		$y =  ($j *  $w1/$nwstar0  *  $hp/$wp); // 100912removed : $starty / $ratiothumb +
		$rcindex = "-c$i-r$j";
		$outfile = "mini/mini" . $rcindex . ".jpg";  // ie: CTr0c2.jpg in mod/split/out/
		$dst_image = imagecreatetruecolor($w1 / $nwstar0, $w1/$nwstar0  *  $hp/$wp);
		imagecopy($dst_image, $src_image, 0, 0, $x, $y, $w1 / $nwstar0, $w1/$nwstar0  *  $hp/$wp);
		imagejpeg($dst_image, $outfile, 100);
		//imagedestroy(dst_image); // rmvd bec gives warning on some servers
	}
}
...
for ($j=0; $j < $nh; $j++) {
	for ($i=0; $i < $nw; $i++) {
		$x = $i * $w0 / $nwstar0 ; //no overlap management necessary
		$y = $starty / $ratiothumb3 + ($j *  $w0/$nwstar0  *  $hp/$wp);
		$rcindex = "-c$i-r$j";
		$outfile = "medium/mini" . $rcindex . ".jpg";  // ie: CT-r0c2.jpg in /medium/
		$dst_image = imagecreatetruecolor($overlaywidth, $overlaywidth  *  $hp/$wp);
		imagecopyresampled($dst_image, $src_image, 0, 0, $x, $y, $overlaywidth, $overlaywidth * $hp/$wp , $w0 / $nwstar0, $w0/$nwstar0  *  $hp/$wp);
		imagejpeg($dst_image, $outfile, 100);
...
?>
		...
		<hr/>
            ...
            <div id="newsSection">
				<form action='pc7.php' method="post" enctype="multipart/form-data" >
                ...
                <p>
					...
					Needed Horizontal Overlap <input name='ox' value ='<?php echo $ox ;?>'  size='2'>in. 
				</p>
				<!-- END h4 module -->
				...
				<h4>
					<span class="subhead">SIZE</span></h4>
                <p>
					<input type="radio" name="fit" value="full" <?php echo $fullchecked; ?> /> Full Width :  

					
					<select name='nw' id='nw'>
							<option value='2'    <?php echo $nw=='2'? 'selected':' '; ?> >2</option>
							<option value='3'    <?php echo $nw=='3'? 'selected':' '; ?> >3</option>
							<option value='4'    <?php echo $nw=='4'? 'selected':' '; ?> >4</option>
							<option value='5'    <?php echo $nw=='5'? 'selected':' '; ?> >5</option>
							<option value='6'    <?php echo $nw=='6'? 'selected':' '; ?> >6</option>
					</select> prints wide. 
					<br/>
					<input type="radio" name="fit" value="frame"  <?php echo $framechecked; ?>  /> Frame Fit : 
					
					<input name='wframe' value ='<?php echo $wframe ;?>'   size='2'>in.   
					<label for='wframe'> frame width </label>
					<!-- <br/>
					<label for='crop'> Crop Vertical </label>
					<input type='checkbox' id='crop' name='crop' <?php echo $cropchecked; ?> > - 
					Start Y: 
					<input name='starty' value ='<?php echo $starty ;?>'  size='2'>px 	
					-->
				</p>
				<!-- END h4 module -->
                ...
                <p>
					...
					(check here to launch the Chopping when everything above looks ok)
					<!--<br />
					Output Folder Name:<input name='pathout' value ='<?php echo $pathout ;?>'  size='15' readonly > 
					--><br />
					Project Name (short, no spaces): <input name='projname' value ='<?php echo $projname ;?>'  size='7'> 	
				</p>
				<!-- END h4 module -->
		...
		<br/>
            ...
            <h2>
                ...
                <p>
				Here you select the picture that's going to be chopped: it can be uploaded from you computer note the name displayed in the field if you want to get back to it later, OR any URL from the internet (like a picture from your Picasa(tm) or Flickr(tm) account)
				</p>
